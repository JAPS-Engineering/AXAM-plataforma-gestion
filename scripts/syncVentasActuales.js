...
async function processFAVE(fave) {
    ...
    try {
        ...
        if (!faveDetails && !primerErrorMostrado) {
            ...
            logError(`   Endpoint: ${(errorInfo && errorInfo.endpoint) || 'N/A'}`);

            if (errorInfo && errorInfo.success === false) {
                logError(`   Error: ${(errorInfo && errorInfo.error) || 'No se pudieron obtener detalles de la FAVE'}`);

                if (errorInfo && errorInfo.errorDetails) {
            ...
            } else if (errorInfo && errorInfo.success === true) {
