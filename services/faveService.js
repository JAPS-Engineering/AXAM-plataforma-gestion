    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
        try {
            ...
            if (error.response?.status === 429) {
                const retryAfter = error.response?.data?.retry || 10; // Segundos a esperar
                logInfo(`  ⏳ Rate limit detectado. Esperando ${retryAfter}s antes del reintento ${attempt + 1}/${maxRetries}...`);

                if (attempt === maxRetries) {
                    logError(`Error al obtener FAVEs (Rate Limit) después de reintentos: ${error.message}`);
                    throw error;
                }
            ...
            logInfo(`  ⚠️ Error al obtener FAVEs. Reintentando en ${waitTime / 1000}s... (Intento ${attempt + 1}/${maxRetries})`);
